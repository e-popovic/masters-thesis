...
for filename in os.scandir(directory):
    if filename.is_file():
        ...
        ssimr = ssim(r1_array, r2_array)
        ssimg = ssim(g1_array, g2_array)
        ssimb = ssim(b1_array, b2_array)

        data = str(ssimr) + ':' + str(ssimg) + ':' + str(ssimb) + '|'

        ssimy = ssim(y1_array, y2_array)
        ssimu = ssim(u1_array, u2_array)
        ssimv = ssim(v1_array, v2_array)

        data += str(ssimy) + ':' + str(ssimu) + ':' + str(ssimv) + '|'

        ssiml = ssim(l1_array, l2_array)
        ssima = ssim(a1_array, a2_array)
        ssimb_lab = ssim(b1_lab_array, b2_lab_array)

        data += str(ssiml) + ':' + str(ssima) + ':' + str(ssimb_lab) + '\n'
        f.write(data)
