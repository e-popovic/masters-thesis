...
for filename in os.scandir(directory):
    if filename.is_file():
        ...
        psnrr = np.sqrt(psnr(r1_array, r2_array))
        psnrg = np.sqrt(psnr(g1_array, g2_array))
        psnrb = np.sqrt(psnr(b1_array, b2_array))

        data = str(psnrr) + ':' + str(psnrg) + ':' + str(psnrb) + '|'

        psnry = np.sqrt(psnr(y1_array, y2_array))
        psnru = np.sqrt(psnr(u1_array, u2_array))
        psnrv = np.sqrt(psnr(v1_array, v2_array))

        data += str(psnry) + ':' + str(psnru) + ':' + str(psnrv) + '|'

        psnrl = np.sqrt(psnr(l1_array, l2_array))
        psnra = np.sqrt(psnr(a1_array, a2_array))
        psnrb_lab = np.sqrt(psnr(b1_lab_array, b2_lab_array))

        data += str(psnrl) + ':' + str(psnra) + ':' + str(psnrb_lab) + '\n'
        f.write(data)
