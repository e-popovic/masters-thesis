...
for filename in os.scandir(directory):
    if filename.is_file():
        ...
        rmser = np.sqrt(mse(r1_array, r2_array))
        rmseg = np.sqrt(mse(g1_array, g2_array))
        rmseb = np.sqrt(mse(b1_array, b2_array))

        data = str(rmser) + ':' + str(rmseg) + ':' + str(rmseb) + '|'

        rmsey = np.sqrt(mse(y1_array, y2_array))
        rmseu = np.sqrt(mse(u1_array, u2_array))
        rmsev = np.sqrt(mse(v1_array, v2_array))

        data += str(rmsey) + ':' + str(rmseu) + ':' + str(rmsev) + '|'

        rmsel = np.sqrt(mse(l1_array, l2_array))
        rmsea = np.sqrt(mse(a1_array, a2_array))
        rmseb_lab = np.sqrt(mse(b1_lab_array, b2_lab_array))

        data += str(rmsel) + ':' + str(rmsea) + ':' + str(rmseb_lab) + '\n'
        f.write(data)
